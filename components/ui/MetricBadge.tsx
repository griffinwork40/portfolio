...
export default function MetricBadge({ value, qualifier, label, className }: MetricBadgeProps) {
  return (
    <div className={cn('inline-flex flex-col gap-0.5', className)}>
      {label && <span className="font-display text-lg text-[--color-accent]">{label}</span>}
      <div className="flex items-baseline gap-1.5">
        <span className="text-base font-bold text-[--color-text]">{value}</span>
      </div>
      <p className="max-w-[240px] text-xs leading-snug text-[--color-muted]">{qualifier}</p>
    </div>
  )
}
